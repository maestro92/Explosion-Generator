...
{
    init(-0.5, 0.5, -0.5, 0.5, 0.0, 0.0, 1.0);
}
...
/*
void EG_QuadModelABS::init1(int w, int h, float cx, float cy, float cw)
{
    /// assigning a quad for rendering textures
    m_modelGeometry = GL_TRIANGLES;
    ...
    float cy1 = cy;  float cy2 = cy+1;
    //1.
    tmp.m_position = glm::vec3(-0.5, h, 0.0);
    tmp.m_UV = glm::vec2(cx1 * cw, cy1 * cw);
    vertices.push_back(tmp);
    //2.
    tmp.m_position = glm::vec3(-0.5, 0.0, 0.0);
    tmp.m_UV = glm::vec2(cx1 * cw, cy2 * cw);
    vertices.push_back(tmp);
