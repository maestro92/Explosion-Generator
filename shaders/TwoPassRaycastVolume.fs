...
void main()
{
    ...
    // recall front-to-back blending is
    // C = α0C0 + (1−α0)a1C1 + (1−α0)(1−α1)a2C2 + (1−α0)(1−α1)(1-α2)α3C3
    // so transparency is the (1−α0)(1−α1)(1-α2) .... term

    float Transparency = 1.0;                       // transparency
    vec3 color = vec3(0.0);                       // iniital color
    ...
    for (int i=0; i < numSamples && travel > 0.0; ++i, pos += step, travel -= stepSize) 
    {
        ...
        else
        {
            ...
            {
                //  Transparency
                color += LightIntensity*density*stepSize * Transparency;
        ...
        color += LightIntensity*density*stepSize * Transparency;    // this is accumulating the colors
